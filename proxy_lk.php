<?php
  ...
  /***************************************************************************
   * Server services this proxy will forward requests to
   * 
   ...
   */
  $serverUrls = array(
    array( 'url' => 'http://localhost:8090', 'matchAll' => true, 'token' => '' ),
    array( 'url' => 'http://localhost:8082', 'matchAll' => true, 'token' => '' ),
    array( 'url' => 'http://localhost:90', 'matchAll' => true, 'token' => '' ),
    array( 'url' => '', 'matchAll' => true, 'token' => '' ),
	array( 'url' => 'http://nefms.nbtsolutions.net', 'matchAll' => true, 'token' => '' )
  );
  /***************************************************************************/
...
$targetUrl = substr($targetUrl,(stripos($targetUrl,"url=")+strlen("url=")));
  //print $targetUrl;
  //echo $targetUrl;
  if (!$targetUrl) {
  ...
  if ($mustMatch) {
    ...
    if ($pos === false) {
      ...
      //echo 'Target URL is not allowed!' .echo $targetPath.'<br/>'. echo $serverUrls. 'Consult the documentation for this proxy to add the target URL to its Whitelist.';
	return;
    }
  }
